...
- **新增 R-21（機密掃描）**：宣告 `tier: shareable` 的 repo 若含雇主標記（brcm/broadcom/airoha/prplOS/BGW720/build20）、個人絕對路徑或憑證模式則 FAIL；`tier: work`/`personal` 視為 not-applicable；自身規則檔/fixtures 與 `.paul-project.yml` 的 `secret_scan.allow` 路徑豁免；豁免 label `policy-exempt:secret-scan`。同時 `.paul-project.yml` 新增 `secret_scan.allow` 設定。
