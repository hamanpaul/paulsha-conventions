...
每個由 `paulsha-conventions` 納管的 repo 必須在 `.paul-project.yml` 宣告 `tier`，值為 `shareable`、`work` 或 `personal` 之一。tier 表達該 repo 內容的預期可見性等級，是機密掃描規則用來決定強制程度的輸入。
...
機密掃描規則（R-21）必須掃描 tracked 文字檔，比對雇主標記 denylist、個人絕對路徑與憑證模式。對宣告 `tier: shareable` 的 repo，命中必須使 policy check 失敗；對 `tier: work` 或 `tier: personal`，命中不得使檢查失敗。
...
由於 denylist 字串必然出現在規則實作、其測試 fixtures、以及被豁免的文件中，R-21 必須將這些路徑排除於掃描之外，使規則不會掃到自己。
